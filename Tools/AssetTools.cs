...
public class AssetTools
{
	...
	[McpTool("sbox_asset_info", "Returns metadata about an asset file.")]
	...
	[McpTool("sbox_asset_duplicate", "Duplicates a GameObject in the scene (deep clone).")]
	...
	[McpTool("sbox_asset_find_missing", "Scans all GameObjects for components with null/missing references.")]
	...
	[McpTool("sbox_asset_bulk_import", "Imports multiple prefabs from a JSON array. Each entry: { path, x?, y?, z? }")]
	...
	[McpTool("sbox_asset_export_json", "Exports a GameObject and its children to JSON.")]
	...
	private static object SerializeGameObject( GameObject go )
	{
		...
		foreach ( var comp in go.Components.GetAll<Component>() )
		{
			...
			var compData = new Dictionary<string, object> { ["type"] = comp.GetType().Name };
