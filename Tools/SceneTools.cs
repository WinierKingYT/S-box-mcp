...
{
	[McpTool("sbox_find_lights", "Lists all light components in the scene.")]
	...
	[McpTool("sbox_set_post_process", "Enables or disables post-processing on the main camera.")]
	...
	[McpTool("sbox_set_tint", "Sets the tint color on a ModelRenderer.")]
	...
	[McpTool("sbox_set_material", "Overrides the material on a ModelRenderer.")]
	...
	[McpTool("sbox_reset_material", "Removes the material override.")]
	...
	[McpTool("sbox_remove_logic_rule", "Removes a Logic Weaver rule by ID.")] public object RemoveLogicRule( string ruleId ) { var r = LogicWeaver.RemoveRule( ruleId ); return r ? new { success = true } : new { error = $"Rule not found: {ruleId}" }; }
	[McpTool("sbox_list_logic_rules", "Lists all Logic Weaver rules.")] public object ListLogicRules() { var rules = LogicWeaver.ListRules().Select( r => new { id = r.Id, triggerType = r.TriggerType, sourceGuid = r.SourceGuid, actionType = r.ActionType, targetGuid = r.TargetGuid, actionParams = r.ActionParams, enabled = r.Enabled } ).ToList(); return new { rules, count = rules.Count }; }
	[McpTool("sbox_clear_logic_rules", "Clears all Logic Weaver rules.")] public object ClearLogicRules() { LogicWeaver.Clear(); return new { success = true }; }

	[McpTool("sbox_navmesh_find_path", "Finds a path between two points on the navmesh.")]
	...
	[McpTool("sbox_terrain_query", "Queries terrain/ground height at a world position. Uses raycast downward.")]
