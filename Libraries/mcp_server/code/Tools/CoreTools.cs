...
public class CoreTools
{
	...
	[McpTool("sbox_run_command", "Executes a console command in the S&box engine.", DestructiveHint = true)]
	...
	[McpTool("sbox_get_object_details", "Returns detailed information about a specific GameObject.", ReadOnlyHint = true)]
	public object GetObjectDetails( string guidStr )
	{
		...
		var components = go.Components.GetAll<Component>().Select( c => { try { var td = TypeLibrary.GetType( c.GetType() ); return new { type = td?.Name ?? "unknown", enabled = c.Enabled }; } catch { return new { type = "unknown", enabled = false }; } } ).ToList();
	...
	[McpTool("sbox_list_assets", "Lists project assets by type.", OptionalParams = new[]{"assetType"}, ReadOnlyHint = true)]
	...
	[McpTool("sbox_get_property", "Reads a property value from a component on a GameObject.", ReadOnlyHint = true)]
	...
	[McpTool("sbox_set_property", "Sets a component property by name. Value is parsed to the correct type automatically.", DestructiveHint = true)]
	...
	[McpTool("sbox_remove_component", "Removes a component from a GameObject.", DestructiveHint = true)]
	...
	[McpTool("sbox_set_component_enabled", "Enables or disables a component on a GameObject.", DestructiveHint = true)]
	...
	[McpTool("sbox_list_components", "Lists all available component types.", OptionalParams = new[]{"filter"}, ReadOnlyHint = true)]
	...
	[McpTool("sbox_duplicate_object", "Duplicates a GameObject.", OptionalParams = new[]{"newName", "offsetX", "offsetY", "offsetZ"}, DestructiveHint = true)]
	...
	[McpTool("sbox_set_parent", "Changes the parent of a GameObject.", DestructiveHint = true)]
	...
	[McpTool("sbox_run_sequence", "Execute multiple tools in sequence. Steps is JSON array of { tool, params } objects.", DestructiveHint = true)]
	...
	[McpTool("sbox_run_batch", "Execute multiple tools sequentially. All steps run regardless of errors. Steps is JSON array of { tool, params }.", DestructiveHint = true)]
	...
	[McpTool("sbox_batch_delete", "Delete multiple GameObjects by GUID. Takes a JSON array of GUID strings.", DestructiveHint = true)]
	...
	[McpTool("sbox_batch_transform", "Set transform on multiple GameObjects. Takes a JSON array of { id, x?, y?, z?, pitch?, yaw?, roll?, scale? }.", DestructiveHint = true)]
	...
	[McpTool("sbox_undo", "Undoes the last action.", DestructiveHint = true)] public object Undo() => UndoRedoManager.Undo();
	[McpTool("sbox_redo", "Redoes the last undone action.", DestructiveHint = true)] public object Redo() => UndoRedoManager.Redo();
	[McpTool("sbox_undo_history", "Lists all undo-able actions.", ReadOnlyHint = true)] public object UndoHistory() { var h = UndoRedoManager.GetHistory(); return new { success = true, count = h.Count, history = h }; }
	[McpTool("sbox_undo_clear", "Clears all undo/redo history.", DestructiveHint = true)] public object UndoClear() { UndoRedoManager.Clear(); return new { success = true, message = "Cleared" }; }

	[McpTool("sbox_config_set", "Save a key-value pair to persistent storage. Values persist across sessions.", DestructiveHint = true)]
	...
	[McpTool("sbox_config_get", "Read a value from persistent storage by key.", ReadOnlyHint = true)]
	...
	[McpTool("sbox_config_list", "List all stored configuration keys.", ReadOnlyHint = true)]
	...
	[McpTool("sbox_config_delete", "Delete a stored configuration value by key.", DestructiveHint = true)]
	...
	[McpTool("sbox_subscribe_event", "Subscribe to game events. Events: phase_change, day_change, alarm, quota_due", ReadOnlyHint = true)]
	...
	[McpTool("sbox_unsubscribe_event", "Unsubscribe from a game event.", ReadOnlyHint = true)]
	...
	[McpTool("sbox_search_assets", "Search for assets by type (prefab, model, material, sound, texture, animation) and name query.", OptionalParams = new[]{"type", "query", "maxResults"}, ReadOnlyHint = true)]
	...
	[McpTool("sbox_list_events", "List all available game event types.", ReadOnlyHint = true)]
	public object ListEvents()
	{
		return new { events = new[] { "phase_change", "day_change", "alarm", "quota_due" } };
	}

	[McpTool("sbox_dry_run", "Validate a tool call without executing it. Checks if tool exists and parses arguments.", OptionalParams = new[]{"paramsJson"}, ReadOnlyHint = true)]
	...
	[McpTool("sbox_read_logs", "Returns recent MCP tool log entries. Capture is manual — engine logs are not available (s&box Log has no hook API).", OptionalParams = new[]{"count", "enable"}, ReadOnlyHint = true)]
	...
	[McpTool("sbox_export_scene", "Export entire scene hierarchy as JSON. Includes transforms, components, and children.", ReadOnlyHint = true)]
	...
	[McpTool("sbox_import_scene", "Import scene hierarchy from JSON. Recreates GameObjects, transforms, components, and hierarchy.", DestructiveHint = true)]
