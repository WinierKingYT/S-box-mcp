...
{
	[McpTool("sbox_debug_inspect", "Returns ALL properties and fields of a GameObject or Component by GUID and optional component index.", OptionalParams = new[]{"componentIndex"}, ReadOnlyHint = true)]
	public object DebugInspect( string guidStr, int componentIndex = -1 )
	{
		...
		{
			try { comps.Add( DumpComponent( c, go ) ); } catch { comps.Add( new { type = "component", error = "Failed to read" } ); }
		}
	...
	private object DumpComponent( Component comp, GameObject go )
	{
		...
		if ( td == null ) return new { type = "unknown", error = "No TypeDescription" };
	...
	[McpTool("sbox_debug_stack", "Returns current call stack info.", ReadOnlyHint = true)]
	...
	[McpTool("sbox_debug_watch", "Watch a GameObject by GUID for N seconds. Uses GameTask.Delay to avoid blocking.", OptionalParams = new[]{"durationSeconds", "intervalMs"}, ReadOnlyHint = true)]
	...
	[McpTool("sbox_debug_list_components", "Lists all component types available in the game.", OptionalParams = new[]{"query"}, ReadOnlyHint = true)]
	...
	[McpTool("sbox_debug_type_info", "Lists all methods, properties, and fields of a TypeLibrary type. Use for API discovery.", ReadOnlyHint = true)]
	...
	[McpTool("sbox_debug_search_types", "Search for registered types by name pattern. Useful for finding correct API types.", OptionalParams = new[]{"query", "category"}, ReadOnlyHint = true)]
