...
{
	[McpTool("sbox_play_sound", "Plays a sound at a position or on a GameObject.", OptionalParams = new[]{"x", "y", "z"}, DestructiveHint = true)]
	...
	[McpTool("sbox_stop_all_sounds", "Stops all playing sounds.", DestructiveHint = true)] public object StopAllSounds() { Sound.StopAll( 0 ); return new { success = true }; }

	[McpTool("sbox_set_camera_fov", "Changes the main camera field of view (10-170 degrees).", DestructiveHint = true)]
	...
	[McpTool("sbox_set_camera_position", "Moves the main camera to a position.", DestructiveHint = true)]
	...
	[McpTool("sbox_reset_camera", "Resets the main camera to default position/rotation.", DestructiveHint = true)]
